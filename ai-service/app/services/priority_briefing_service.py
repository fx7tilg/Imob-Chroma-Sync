...
RULES for Depth:
- EXTREME DEPTH: You are on an unrestricted token tier. Provide highly extensive, deeply detailed analysis.
- Use ONLY the data provided. NEVER invent problems, risks, or metrics.
- Write in a highly professional, enterprise-grade tone. Provide an in-depth explanation that leaves no detail behind.
- For the executive summary, provide a thorough multi-paragraph overview of the project health, major risks, and strategic directions.
- For the finding narratives, provide a deep, rich explanation of why the finding matters, how it impacts the project, and the best solution to resolve it. Connect findings to real-world manufacturing impact.
- Be specific: name component names, colour codes, team names from the data.
- No emoji. No marketing language.

Return a JSON object:
{
  "executive_summary": "A deep, professional multi-paragraph overview of project health, current status, and required improvements. Format with extensive markdown (bolding, lists, blockquotes).",
  "finding_narratives": {
    "<finding_id>": "A comprehensive, deep analysis of this specific finding, its impact, and recommended best solution. Use extensive markdown."
  },
  "priority_actions": [
    "Concrete, highly-detailed, high-impact strategic action 1",
    "Concrete, highly-detailed, high-impact strategic action 2"
  ]
}
...
RULES for Depth:
- EXTREME DEPTH: Provide highly extensive, deeply detailed analysis of the risks and their cascading impact.
- Do NOT hallucinate. Do not invent risks, people, dates, or teams. If data isn't there, state it's not recorded.
- Write in a highly professional, enterprise PLM tone. Be exceptionally specific and thorough.
- Differentiate clearly between "Already being done" and "Still needs attention".
- STRICTLY use numerical digits for counts (e.g., use "5" instead of "five").

Return ONLY a JSON object:
{
  "executive_summary": "A deep, comprehensive executive risk posture overview (Critical/High/Blocked/Overdue). Use extensive markdown.",
  "top_5_risks": [
    {
      "rank": 1,
      "title": "Clear risk title",
      "severity": "critical or attention",
      "what_is_the_risk": "Deep explanation of the core issue and root cause.",
      "why_is_it_critical": "Detailed explanation of why this matters and the real-world manufacturing impact.",
      "already_being_done": "Detailed list of what actions have been recorded so far.",
      "still_needs_attention": "The exact unresolved issues in depth.",
      "impact": "Detailed impact on downstream processes.",
      "why_now": "Detailed explanation of why this is urgent.",
      "next_action": "Concrete, highly-detailed required action.",
      "affected_teams": ["team1", "team2"],
      "affected_decisions": ["ID1", "ID2"]
    }
  ],
  "what_needs_attention_first": "A clear, deep, evidence-based action sequence. Use extensive markdown."
}
...
Your task is to perform an EXTREMELY DEEP, ENTERPRISE-GRADE ANALYSIS of ONE specific submitted decision and its entire data flow.
The user requires an uncompromising, granular, real-world "depth touch" explanation. Do not just summarize. You must explain the REAL-WORLD IMPACT of every single field, specification, timeline, and conflict on manufacturing, supply chain, quality testing, and aesthetic harmony.

RULES for Depth:
- EXTREME DEPTH: You are on an unrestricted token tier. Provide highly extensive, deeply detailed analysis. Leave no stone unturned.
- EXTRACT AND ANALYZE EVERY FIELD. Connect data to reality (e.g. how a missing part number halts procurement).
- Do NOT hallucinate. Use ONLY the provided context.
- Use a highly premium, authoritative, enterprise-grade tone. Use advanced markdown (tables, blockquotes, bolding, lists) to make the text design stunning and readable.

Return ONLY a JSON object exactly matching this structure (all strings must be rich, extensive markdown):
{
  "executive_summary": "A powerful, multi-paragraph enterprise summary of the decision's true state, its real-world implications, and core blockers.",
  "who_is_doing_what": [
    { "role": "Team/Role", "current_state": "e.g., Submitted/Reviewing/Waiting", "what_they_did": "...", "what_they_need": "..." }
  ],
  "version_analysis": {
    "current_version": "vX",
    "status": "...",
    "history_markdown": "Deep, extensive explanation of version progression and how the data flow has evolved."
  },
  "workflow_analysis": "Extensive markdown analyzing the stage progression, exact blockers, and real-world timeline impacts.",
  "approval_analysis": "Deep markdown inspecting approval state, who approved, who rejected, and the specific feedback.",
  "conflict_analysis": "Extensive markdown explaining the root cause of conflicts and real-world consequences if ignored.",
  "readiness_analysis": "Deep markdown explaining the deterministic score, exactly what passed/failed, and why those failures matter.",
  "risk_analysis": {
    "already_being_done": "Markdown list of actual recorded mitigating actions in great detail.",
    "still_needs_attention": "Markdown list of actual unresolved items and cascading impact in great detail."
  },
  "dependency_analysis": "Extensive markdown showing relationships to other decisions and how they bottleneck the schedule.",
  "activity_analysis": "Extensive markdown explaining behavioral patterns from the audit history.",
  "feedback_analysis": "Deep markdown detailing unresolved vs resolved comments and design/engineering notes.",
  "what_to_check_by_role": [
    { "role": "Team/Role", "guidance": "Markdown of exact, granular evidence to inspect." }
  ],
  "next_actions": [
    "One highly-detailed, deep step to unblock the flow",
    "Another highly-detailed, deep step to unblock the flow"
  ]
}
...
async def briefing(req: PriorityBriefingRequest) -> PriorityBriefingResponse:
    ...
    for d in req.decisions:
        compact.append({
            ...
            "engineering_notes": d.get("engineering_notes"),
            "procurement_notes": d.get("procurement_notes"),
            "quality_notes": d.get("quality_notes"),
            "reference_documents": d.get("reference_documents", []),
        })
