-- Migration: 20260908000004_role_decisions.sql
-- Description: Add procurement_decision and quality_decision to decisions table and their respective lookup tables.

-- 1. Create lookup tables and populate them
CREATE TABLE IF NOT EXISTS master_procurement_decisions (
    code text PRIMARY KEY,
    label text NOT NULL,
    is_active boolean DEFAULT true,
    created_at timestamptz DEFAULT now() NOT NULL
);
ALTER TABLE master_procurement_decisions ENABLE ROW LEVEL SECURITY;
CREATE POLICY "master_proc_dec_select" ON master_procurement_decisions FOR SELECT TO authenticated USING (true);
CREATE POLICY "master_proc_dec_all" ON master_procurement_decisions FOR ALL TO authenticated USING (is_project_lead());
INSERT INTO master_procurement_decisions (code, label) VALUES
    ('APPROVED', 'Approved'),
    ('CHANGES_REQUIRED', 'Changes Required'),
    ('REJECTED', 'Rejected')
ON CONFLICT (code) DO NOTHING;

CREATE TABLE IF NOT EXISTS master_quality_decisions (
    code text PRIMARY KEY,
    label text NOT NULL,
    is_active boolean DEFAULT true,
    created_at timestamptz DEFAULT now() NOT NULL
);
ALTER TABLE master_quality_decisions ENABLE ROW LEVEL SECURITY;
CREATE POLICY "master_qual_dec_select" ON master_quality_decisions FOR SELECT TO authenticated USING (true);
CREATE POLICY "master_qual_dec_all" ON master_quality_decisions FOR ALL TO authenticated USING (is_project_lead());
INSERT INTO master_quality_decisions (code, label) VALUES
    ('APPROVED', 'Approved'),
    ('CHANGES_REQUIRED', 'Changes Required'),
    ('REJECTED', 'Rejected')
ON CONFLICT (code) DO NOTHING;

-- 2. Add columns to decisions
ALTER TABLE decisions ADD COLUMN IF NOT EXISTS procurement_decision text REFERENCES master_procurement_decisions(code);
ALTER TABLE decisions ADD COLUMN IF NOT EXISTS quality_decision text REFERENCES master_quality_decisions(code);

-- 3. Update column guard
