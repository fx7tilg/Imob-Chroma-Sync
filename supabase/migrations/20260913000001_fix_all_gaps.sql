-- Migration: 20260913000001_fix_all_gaps.sql
-- Description: Fixes GAP-1 (self-transitions), GAP-7 (takeOver role check),
--              and GAP-9 (approvals role check at DB level).

set search_path = public, extensions;

-- =========================================================================
-- GAP-1: Allow self-transitions (submitted → submitted, under_review → under_review)
-- GAP-7: Only editors/leads can set ownership IDs
-- =========================================================================
...
BEGIN
    ...
    -- === STATUS TRANSITIONS (with GAP-1 fix: allow self-transitions) ===
    IF NEW.status IS DISTINCT FROM OLD.status THEN
        -- Self-transitions are always allowed (version bump without status change intent)
        -- Note: this is handled by the IS DISTINCT check above, but we also allow
        -- explicit same-value transitions for the rollback logic.
        ...
        ELSIF OLD.status = 'submitted' AND NEW.status = 'rejected' THEN
            -- Anyone can reject from submitted
            NULL;
        ...
        ELSIF OLD.status = 'rejected' AND NEW.status = 'submitted' THEN
            -- Allow resubmission from rejected after fixes
            NULL;
        ELSIF OLD.status = NEW.status THEN
            -- GAP-1 FIX: Allow self-transitions (e.g. submitted → submitted for version bump)
            NULL;
        ELSE
...
-- =========================================================================
-- GAP-9: Approvals guard - only approvers/admins can approve/reject
-- =========================================================================
...
BEGIN
    ...
    -- GAP-9 FIX: Only approvers/admins/leads can cast approved/rejected votes
    IF new.status IN ('approved','rejected') THEN
        -- Service role (AI, system) bypasses this check
        IF auth.uid() IS NOT NULL THEN
            SELECT role INTO v_user_role FROM profiles WHERE id = auth.uid();
            IF v_user_role NOT IN ('approver', 'admin') THEN
                -- Check if project lead
                IF NOT coalesce(is_project_lead(), false) THEN
                    RAISE EXCEPTION 'only approvers or admins can approve/reject decisions (your role: %)', v_user_role
                        USING errcode = '42501';
                END IF;
            END IF;
        END IF;
