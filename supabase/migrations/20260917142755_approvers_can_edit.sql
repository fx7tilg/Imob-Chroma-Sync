-- Migration: 20260917142755_approvers_can_edit.sql
-- Description: Allow approvers to edit and take over decisions

set search_path = public, extensions;

-- Update column guard to allow Approvers to edit and take over
...
BEGIN
    ...
    -- === GAP-7: TAKE OVER / OWNERSHIP LOCKS — only editors/leads/approvers can take over ===
        ...
        IF NEW.engineering_owner_id IS NOT NULL THEN
            IF NEW.engineering_owner_id <> auth.uid() THEN
                RAISE EXCEPTION 'you can only take over a decision for yourself'
                    USING errcode = '42501';
            END IF;
            -- Verify the user has editor or approver role
            SELECT role INTO v_user_role FROM profiles WHERE id = auth.uid();
            IF v_user_role NOT IN ('editor', 'approver', 'admin') THEN
                RAISE EXCEPTION 'only editors or approvers can take over form ownership (your role: %)', v_user_role
                    USING errcode = '42501';
            END IF;
        END IF;
    END IF;
