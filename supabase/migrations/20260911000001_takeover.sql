-- =========================================================================
-- Chroma Sync - Take Over & Queue Mechanism
-- Migration: Add ownership columns and enforce locks via column guard
-- =========================================================================
...
ALTER TABLE decisions ADD COLUMN IF NOT EXISTS engineering_owner_id uuid REFERENCES auth.users(id);
ALTER TABLE decisions ADD COLUMN IF NOT EXISTS procurement_owner_id uuid REFERENCES auth.users(id);
ALTER TABLE decisions ADD COLUMN IF NOT EXISTS quality_owner_id uuid REFERENCES auth.users(id);

-- =========================================================================
-- 2. UPDATE COLUMN GUARD
-- =========================================================================
...
BEGIN
    ...
    -- If a decision is "owned" by someone in a role, ONLY that person can edit the role's fields.
    -- Furthermore, if they try to change the owner, they can only release it (set to NULL).
    
    -- Engineering Ownership
    IF OLD.engineering_owner_id IS NOT NULL THEN
        -- If eng fields changed, current user MUST be the owner
        IF v_eng_changed AND OLD.engineering_owner_id <> auth.uid() THEN
            RAISE EXCEPTION 'only the engineering owner (%) can edit engineering fields', OLD.engineering_owner_id USING errcode = '42501';
        END IF;
    END IF;
    ...
    IF NEW.engineering_owner_id IS DISTINCT FROM OLD.engineering_owner_id THEN
        IF NEW.engineering_owner_id IS NOT NULL AND NEW.engineering_owner_id <> auth.uid() THEN
            RAISE EXCEPTION 'you can only take over a decision for yourself' USING errcode = '42501';
        END IF;
    END IF;
    ...
    IF OLD.procurement_owner_id IS NOT NULL THEN
        IF v_proc_changed AND OLD.procurement_owner_id <> auth.uid() THEN
            RAISE EXCEPTION 'only the procurement owner (%) can edit procurement fields', OLD.procurement_owner_id USING errcode = '42501';
        END IF;
    END IF;
    ...
    IF OLD.quality_owner_id IS NOT NULL THEN
        IF v_qual_changed AND OLD.quality_owner_id <> auth.uid() THEN
            RAISE EXCEPTION 'only the quality owner (%) can edit quality fields', OLD.quality_owner_id USING errcode = '42501';
        END IF;
    END IF;
